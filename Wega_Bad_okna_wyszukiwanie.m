...
f_sygnal_2 = fo + f_syg_1 + del;
s = sin(2 * pi * f_sygnal_1 * n / fs) + sin(2 * pi * (f_sygnal_2) * n / fs) + noise;         % Próbki sygna³u - podpróbkowanie
df = abs(abs(f_sygnal_1) - abs(f_sygnal_2));
sx = s .* norm;                                      % Normowanie sygna³u                   
S = abs(fft(sx, N));                                 % Modu³ widma sygna³u 
%S = fft(sx, N);

[maxRect, maxId] = max(S(1:512));
if ((S(maxId) > S(maxId + 1)) && (S(maxId + 2) > S(maxId + 1)) && (S(maxId) > 0.01) && (S(maxId + 2) > 0.01)) || ((S(maxId) > S(maxId - 1)) && (S(maxId - 2) > S(maxId - 1)) && (S(maxId) > 0.01) && (S(maxId - 2) > 0.01))   
   maxRect
   maxId
    break;
end
...
grid on;
title('Widmo Amplitudowe z oknem Prostok¹tnym');
...
s = sin(2 * pi * f_sygnal_1 * n / fs) + sin(2 * pi * (f_sygnal_2) * n / fs) + noise;         % Próbki sygna³u - podpróbkowanie
df = abs(abs(f_sygnal_1) - abs(f_sygnal_2));
sx = s .* O_Hanning;                                      % Normowanie sygna³u                   
S = abs(fft(sx, N));                                 % Modu³ widma sygna³u 
%S = fft(sx, N);
...
xticklabels(f_lab_zoom)
grid on;
title('Widmo Amplitudowe z oknem Hanninga');
xlabel('Czêstotliwoœæ unormowana [kHz]');
ylabel('Wartoœæ próbki');
if legenda == 1
    legend(['df = ', num2str(df), ' [Hz],  ']);
...
s = sin(2 * pi * f_sygnal_1 * n / fs) + sin(2 * pi * (f_sygnal_2) * n / fs) + noise;         % Próbki sygna³u - podpróbkowanie
df = abs(abs(f_sygnal_1) - abs(f_sygnal_2));
sx = s .* O_Hamming;                                      % Normowanie sygna³u                   
S = abs(fft(sx, N));                                 % Modu³ widma sygna³u 
%S = fft(sx, N);
...
xticklabels(f_lab_zoom)
grid on;
title('Widmo Amplitudowe z oknem Hamminga');
xlabel('Czêstotliwoœæ unormowana [kHz]');
ylabel('Wartoœæ próbki');
if legenda == 1
    legend(['df = ', num2str(df), ' [Hz],  ']);
...
s = sin(2 * pi * f_sygnal_1 * n / fs) + sin(2 * pi * (f_sygnal_2) * n / fs) + noise;         % Próbki sygna³u - podpróbkowanie
df = abs(abs(f_sygnal_1) - abs(f_sygnal_2));
sx = s .* O_Blackman;                                      % Normowanie sygna³u                   
S = abs(fft(sx, N));                                 % Modu³ widma sygna³u 
%S = fft(sx, N);
...
xticklabels(f_lab_zoom)
grid on;
title('Widmo Amplitudowe z oknem Blackmana');
xlabel('Czêstotliwoœæ unormowana [kHz]');
ylabel('Wartoœæ próbki');
if legenda == 1
    legend(['df = ', num2str(df), ' [Hz],  ']);
